...
type CityParcelOption = (typeof cityParcelTypes)[number];
type OutsideParcelOption = (typeof outsideParcelTypes)[number];
type ParcelOption = (typeof cityParcelTypes)[number] | (typeof outsideParcelTypes)[number];
type CityParcelKey = CityParcelOption['key'];
type OutsideParcelKey = OutsideParcelOption['key'];
...
const supportedIntercityHubs = {
  'dar es salaam': { label: 'Dar es Salaam', latitude: -6.7924, longitude: 39.2083 },
  mwanza: { label: 'Mwanza', latitude: -2.5164, longitude: 32.9175 },
  arusha: { label: 'Arusha', latitude: -3.3869, longitude: 36.683 },
  mbeya: { label: 'Mbeya', latitude: -8.9094, longitude: 33.4607 },
  morogoro: { label: 'Morogoro', latitude: -6.8235, longitude: 37.6613 },
  mtwara: { label: 'Mtwara', latitude: -10.2667, longitude: 40.1833 },
} as const;

type Hub = (typeof supportedIntercityHubs)[keyof typeof supportedIntercityHubs];

const cityParcelTypeMultipliers = {
  document: 1,
  food: 1.08,
  box: 1.18,
  fragile: 1.25,
...
const outsideParcelTypeMultipliers = {
  luggage: 1.28,
  'box-bulk': 1.18,
  electronics: 1.32,
} as const;
...
function formatTzsRange(minAmount: number, maxAmount: number) {
  return `${formatTzs(minAmount)} - ${formatTzs(maxAmount)}`;
}

function formatHoursFromDistance(distanceKm: number) {
  const travelHours = Math.max(3, Math.round((distanceKm / 58) * 10) / 10);
  const low = Math.max(3, Math.floor(travelHours));
  const high = Math.max(low + 1, Math.ceil(travelHours + 1));
  return `${low}-${high} hrs`;
}

function getNearestHub(point: GeoPoint): Hub | undefined {
  return Object.values(supportedIntercityHubs).reduce((closest, hub) => {
    const distance = getDistanceBetweenPoints(point, hub);
    if (!closest || distance < closest.distance) {
      return { hub, distance };
    }
    return closest;
  }, null as { hub: Hub; distance: number } | null)?.hub;
}

function getHubFromText(value: string): Hub | undefined {
  const normalized = value.trim().toLowerCase();
  const matchedEntry = Object.entries(supportedIntercityHubs).find(
    ([key, hub]) => normalized.includes(key) || normalized.includes(hub.label.toLowerCase())
  );
...
function buildParcelPricing(params: {
  scope: ParcelScope;
  parcelType: ParcelOption['key'];
  pickupPoint: GeoPoint;
  dropoffPoint: GeoPoint | null;
  dropoffText: string;
}) {
  const { scope, parcelType, pickupPoint, dropoffPoint, dropoffText } = params;

  if (scope === 'city') {
    const destinationPoint = dropoffPoint ?? pickupPoint;
    const distanceKm = Math.max(getDistanceBetweenPoints(pickupPoint, destinationPoint) / 1000, 1.5);
    const baseFare = 2200;
    const distanceFare = distanceKm * 900;
    const cityParcelType = parcelType as CityParcelKey;
    const typeMultiplier = cityParcelTypeMultipliers[cityParcelType] ?? 1;
    const total = (baseFare + distanceFare) * typeMultiplier;

    return {
      priceLabel: formatTzs(total),
      etaLabel: distanceKm <= 5 ? '15-25 min' : distanceKm <= 12 ? '25-40 min' : '40-60 min',
      routeLabel: 'Dar es Salaam local delivery',
    };
  }

  const originHub = getNearestHub(pickupPoint) ?? supportedIntercityHubs['dar es salaam'];
  const destinationHub = getHubFromText(dropoffText) ?? (dropoffPoint ? getNearestHub(dropoffPoint) : null) ?? supportedIntercityHubs.morogoro;
  const routeDistanceKm = Math.max(getDistanceBetweenPoints(originHub, destinationHub) / 1000, 1);
  const darMwanzaReferenceKm = getDistanceBetweenPoints(supportedIntercityHubs['dar es salaam'], supportedIntercityHubs.mwanza) / 1000;
  const baselineFare = 12000;
  const outsideParcelType = parcelType as OutsideParcelKey;
  const typeMultiplier = outsideParcelTypeMultipliers[outsideParcelType] ?? 1;
  const coreFare = (routeDistanceKm / darMwanzaReferenceKm) * baselineFare;
  const minFare = coreFare * typeMultiplier;
  const maxFare = minFare * 1.18;

  return {
    priceLabel: formatTzsRange(minFare, maxFare),
    etaLabel: formatHoursFromDistance(routeDistanceKm),
    routeLabel: `${originHub.label} to ${destinationHub.label}`,
  };
}
...
export default function SendParcelScreen() {
  ...
  const pricing = useMemo(
    ...
    [activePackage.key, dropoff, dropoffPoint, pickupPoint, selectedScope]
  );
  ...
  const applySavedPlaceToPickup = async (place: SavedPlace) => {
    try {
      ...
      Alert.alert('Location not found', 'We could not place this saved pickup on the map. Try a clearer address.');
    }
  };
  ...
  const applySavedPlaceToDropoff = async (place: SavedPlace) => {
    ...
    try {
      ...
      Alert.alert('Location not found', 'We could not place this saved destination on the map. Try a clearer address.');
    }
  };
